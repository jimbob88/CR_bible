...
def get_args():
    ...
    parser.add_argument('--xmlname', '-x', required=True, help="select the name of the xml file 'vernacularParms'")
    parser.add_argument('--out', '-o', default='dataset.json', help="the name of the output json")
...
def filter_folder(folder: Path, regex: str) -> List[Path]:
    ...
    return [element for element in folder.iterdir() if element.is_file() and re.search(regex, element.name)]
...
def paragraph_carriage_returns(soup: bs4.BeautifulSoup):
    ...
    paragraph_verses = [
        [int(verse.get("id")[1:]) for verse in verses(paragraph)] for paragraph in paragraphs(soup)
    ]
...
if __name__ == '__main__':
    ...
    filename_regex = htm_filename_regex(list(conversion_table.keys()), [])
